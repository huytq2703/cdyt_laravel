...
const onClickAddNew = () => {
  Inertia.get(route(props.rc_Page));
};
...
const onSearch = () => {
  filtersForm.get(route(props.rl_Page), {
    preserveState: true,
    replace: true,
  });
};
...
const deletePost = (details) => {
  confirm.require({
    ...
    rejectLabel: "Huỷ",
    accept: () => {
      Inertia.delete(route(props.rad_Page, { id: details.id }));
    },
    reject: () => {},
  });
};
...
const onPageChange = ({ page }) => {
  Inertia.get(route(props.rl_Page), { page: page + 1, search: props?.params?.search });
};
...
<template>
  <AdminLayout>
    <Head title="Quản lý nội dung trang tĩnh" />

    <div class="card">
      <h5 class="font-bold">Danh sách các trang tĩnh</h5>
